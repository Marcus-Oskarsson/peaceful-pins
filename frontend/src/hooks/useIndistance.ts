...
export function useIndistance(postPosition: {
  latitude: number;
  longitude: number;
}) {
  const MAX_DISTANCE = 0.2; // 200m
  const { latitude, longitude, accuracy, error } = usePosition();
  if (error || !latitude || !longitude || !accuracy)
    return { inDistance: false, accuracy: accuracy || 0 };
  ...
  return { inDistance: distance <= MAX_DISTANCE + accuracy, accuracy };
}
